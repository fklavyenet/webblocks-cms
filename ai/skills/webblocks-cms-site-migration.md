...
- For image-heavy migrations, prefer discovered `POST /webadmin/api/media/fetch` when available to import approved public source media into normal Media Library records before building gallery blocks. On older installs without that endpoint, use an explicit migration-side download/upload mapping step.
- On Herne installs where the site root is owned by the web runtime, run migration API writes through the web server/runtime instead of booting Laravel HTTP internals as `deploy`; direct CLI requests can fail when `storage/logs` is not writable by `deploy`.
- Before DNS/SSL is live, local target API calls can still exercise the real web runtime with HTTPS host resolution, for example by resolving the target host to `127.0.0.1` and using a temporary trusted API token without printing it.
- Large source images can hit nginx upload limits during Media Library import. Prefer source-generated resized variants where available, and record any server upload-size gap separately from CMS block capability gaps.
