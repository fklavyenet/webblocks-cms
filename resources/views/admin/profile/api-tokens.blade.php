...
@section('content')
    ...
    @if ($createdToken)
        <div class="wb-alert wb-alert-success"><div class="wb-stack wb-gap-3">
            ...
            <div class="wb-stack wb-gap-2"><div class="wb-cluster wb-gap-2"><label class="wb-label" for="created_personal_api_token">{{ $systemText('full_token') }}</label><button type="button" class="wb-btn wb-btn-ghost wb-btn-sm wb-btn-icon" data-wb-copy-target="created_personal_api_token" aria-label="{{ $systemText('copy_full_token') }}"><i class="wb-icon wb-icon-copy" aria-hidden="true"></i></button><span class="wb-text-sm" data-wb-api-token-copy-feedback data-copy-success="{{ $systemText('copied') }}" data-copy-failed="{{ $systemText('copy_failed') }}" role="status" aria-live="polite"></span></div><textarea id="created_personal_api_token" class="wb-textarea" rows="2" readonly>{{ $createdToken }}</textarea></div>
            <div class="wb-stack wb-gap-2"><div class="wb-cluster wb-gap-2"><label class="wb-label" for="created_personal_api_token_env">{{ $systemText('env_example') }}</label><button type="button" class="wb-btn wb-btn-ghost wb-btn-sm wb-btn-icon" data-wb-copy-target="created_personal_api_token_env" aria-label="{{ $systemText('copy_env_example') }}"><i class="wb-icon wb-icon-copy" aria-hidden="true"></i></button><span class="wb-text-sm" data-wb-api-token-copy-feedback data-copy-success="{{ $systemText('copied') }}" data-copy-failed="{{ $systemText('copy_failed') }}" role="status" aria-live="polite"></span></div><textarea id="created_personal_api_token_env" class="wb-textarea" rows="3" readonly>WEBBLOCKS_CMS_API_URL={{ $apiBaseUrl }}
WEBBLOCKS_CMS_API_TOKEN={{ $createdToken }}</textarea></div>
            <div class="wb-stack wb-gap-2"><div class="wb-cluster wb-gap-2"><label class="wb-label" for="created_personal_api_token_prompt">{{ $systemText('ai_setup_prompt') }}</label><button type="button" class="wb-btn wb-btn-ghost wb-btn-sm wb-btn-icon" data-wb-copy-target="created_personal_api_token_prompt" aria-label="{{ $systemText('ai_setup_prompt') }}"><i class="wb-icon wb-icon-copy" aria-hidden="true"></i></button><span class="wb-text-sm" data-wb-api-token-copy-feedback data-copy-success="{{ $systemText('copied') }}" data-copy-failed="{{ $systemText('copy_failed') }}" role="status" aria-live="polite"></span></div><textarea id="created_personal_api_token_prompt" class="wb-textarea" rows="5" readonly>{{ $systemText('ai_setup_prompt_text') }}

WEBBLOCKS_CMS_API_URL={{ $apiBaseUrl }}
WEBBLOCKS_CMS_API_TOKEN={{ $createdToken }}</textarea></div>
        </div></div>
    @endif
    ...
    <section class="wb-card">
        ...
        <form method="POST" action="{{ route('admin.profile.api-tokens.store') }}">@csrf
            <div class="wb-card-body wb-stack wb-gap-4">
                ...
                <div class="wb-field wb-stack wb-gap-3"><div class="wb-label">{{ $text('network_controls') }}</div><div><label class="wb-label" for="personal_api_token_allowed_ips">{{ $text('allowed_ips') }}</label><div class="wb-text-sm wb-text-muted">{{ $text('allowed_ips_help') }}</div><textarea id="personal_api_token_allowed_ips" name="allowed_ip_ranges" class="wb-textarea" rows="3" placeholder="203.0.113.10&#10;2001:db8::/32">{{ implode("\n", (array) old('allowed_ip_ranges', [])) }}</textarea>@error('allowed_ip_ranges.*')<div class="wb-field-error">{{ $message }}</div>@enderror</div><div><label class="wb-label" for="personal_api_token_rate_limit">{{ $text('rate_limit') }}</label><select id="personal_api_token_rate_limit" name="requests_per_minute" class="wb-select">@foreach ([30, 60, 120, 300] as $limit)<option value="{{ $limit }}" @selected((int) old('requests_per_minute', 60) === $limit)>{{ $limit }}</option>@endforeach</select></div></div>
            </div>
            ...
            <div class="wb-modal-dialog">
                ...
                <form method="POST" action="{{ route('admin.profile.api-tokens.update', $token) }}">@csrf @method('PUT')
                    <div class="wb-modal-body wb-stack wb-gap-4 wb-api-token-modal-body">
                        ...
                        <div class="wb-field wb-stack wb-gap-3"><div class="wb-label">{{ $text('network_controls') }}</div><div><label class="wb-label" for="edit_personal_api_token_allowed_ips_{{ $token->id }}">{{ $text('allowed_ips') }}</label><div class="wb-text-sm wb-text-muted">{{ $text('allowed_ips_help') }}</div><textarea id="edit_personal_api_token_allowed_ips_{{ $token->id }}" name="allowed_ip_ranges" class="wb-textarea" rows="3">{{ implode("\n", $token->allowed_ip_ranges ?? []) }}</textarea></div><div><label class="wb-label" for="edit_personal_api_token_rate_limit_{{ $token->id }}">{{ $text('rate_limit') }}</label><select id="edit_personal_api_token_rate_limit_{{ $token->id }}" name="requests_per_minute" class="wb-select">@foreach ([30, 60, 120, 300] as $limit)<option value="{{ $limit }}" @selected((int) ($token->requests_per_minute ?: 60) === $limit)>{{ $limit }}</option>@endforeach</select></div></div>
                    </div>
