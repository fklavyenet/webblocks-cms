@php
    $submittedGalleryIds = old("{$prefix}.gallery_media_ids", old("{$prefix}.gallery_asset_ids", $block->galleryMediaIds()));
@endphp
...
<div class="wb-grid wb-grid-3">
    @for ($galleryIndex = 0; $galleryIndex < 3; $galleryIndex++)
        <div class="wb-stack wb-gap-1">
            <label for="block_{{ $index }}_gallery_media_{{ $galleryIndex }}">Gallery Media {{ $galleryIndex + 1 }}</label>
            <input id="block_{{ $index }}_gallery_media_{{ $galleryIndex }}" name="{{ $prefix }}[gallery_media_ids][]" class="wb-input" type="number" min="1" value="{{ $submittedGalleryIds[$galleryIndex] ?? '' }}">
        </div>
    @endfor
</div>
