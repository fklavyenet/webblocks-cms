    ...
    >
        ...
        <td class="wb-block-hierarchy-cell wb-admin-slot-block-type-cell">
            <div class="wb-block-hierarchy">
                <div class="wb-cms-block-tree-item">
                    <button type="button" class="wb-action-btn" data-admin-sortable-handle aria-label="{{ $inlineBlocksText('drag_to_reorder') }}" title="{{ $inlineBlocksText('drag_to_reorder') }}">
                    ...
                    @if ($hasChildren)
                        <button
                            ...
                            aria-label="{{ $isExpanded ? $inlineBlocksText('collapse_children') : $inlineBlocksText('expand_children') }}"
                            title="{{ $isExpanded ? $inlineBlocksText('collapse_children') : $inlineBlocksText('expand_children') }}"
                        >
        ...
        <td class="wb-cms-block-children-cell">
            @if ($canAddChildren || $hasChildren)
                <span class="wb-cms-block-children-badge" aria-label="{{ $childCount }} {{ $childCount === 1 ? $inlineBlocksText('child_block') : $inlineBlocksText('child_blocks') }}">{{ $childCount }}</span>
            @else
        ...
        <td class="wb-admin-slot-block-actions-cell">
            <div class="wb-action-group">
                ...
                @if ($canAddChildren)
                    <a href="{{ $slotBlockRoute(['picker' => 1, 'parent_id' => $block->id]) }}" class="wb-action-btn" title="{{ $inlineBlocksText('add_child') }}" aria-label="{{ $inlineBlocksText('add_child') }}" data-wb-slot-block-link data-base-url="{{ $slotBlockBaseRoute(['picker' => 1, 'parent_id' => $block->id]) }}"><i class="wb-icon wb-icon-plus" aria-hidden="true"></i></a>
                @endif
                <a href="{{ $slotBlockRoute(['delete' => $block->id]) }}" class="wb-action-btn wb-action-btn-delete" title="{{ $inlineBlocksText('delete') }}" aria-label="{{ $inlineBlocksText('delete') }}" data-wb-slot-block-link data-base-url="{{ $slotBlockBaseRoute(['delete' => $block->id]) }}"><i class="wb-icon wb-icon-trash" aria-hidden="true"></i></a>
            </div>
        </td>
    </tr>
</tbody>
