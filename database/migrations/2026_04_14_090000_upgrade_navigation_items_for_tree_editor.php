...
return new class extends Migration
{
  public function up(): void
  {
    Schema::table('wbcms_navigation_items', function (Blueprint $table) {
      ...
      if (Schema::hasColumn('wbcms_navigation_items', 'sort_order')) {
        $table->renameColumn('sort_order', 'position');
      }
    });

    Schema::table('wbcms_navigation_items', function (Blueprint $table) {
      if (! Schema::hasColumn('wbcms_navigation_items', 'link_type')) {
        $table->string('link_type')->default('custom_url')->after('title');
      }

      if (! Schema::hasColumn('wbcms_navigation_items', 'visibility')) {
        $table->string('visibility')->default('visible')->after('target');
      }

      if (! Schema::hasColumn('wbcms_navigation_items', 'is_system')) {
        $table->boolean('is_system')->default(false)->after('visibility');
      }
    });

    DB::table('wbcms_navigation_items')->where('menu_key', 'header')->update(['menu_key' => 'primary']);
    DB::table('wbcms_navigation_items')->where('menu_key', 'primary')->whereNull('position')->update(['position' => 0]);
    DB::table('wbcms_navigation_items')->whereNull('menu_key')->update(['menu_key' => 'primary']);
    ...
    if (Schema::hasColumn('wbcms_navigation_items', 'is_active')) {
      DB::table('wbcms_navigation_items')->where('is_active', true)->update(['visibility' => 'visible']);
      DB::table('wbcms_navigation_items')->where('is_active', false)->update(['visibility' => 'hidden']);
  ...
  public function down(): void
  {
    if (Schema::hasColumn('wbcms_navigation_items', 'visibility')) {
      ...
      DB::table('wbcms_navigation_items')->where('visibility', 'visible')->update(['is_active' => true]);
      DB::table('wbcms_navigation_items')->where('visibility', 'hidden')->update(['is_active' => false]);
    }
    ...
    Schema::table('wbcms_navigation_items', function (Blueprint $table) {
      ...
      if (Schema::hasColumn('wbcms_navigation_items', 'position')) {
