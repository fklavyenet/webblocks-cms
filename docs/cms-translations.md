...
Use `php artisan webblocks:admin-translation-audit --locale=de --native-only --strict` as the bridge-removal readiness check. This mode intentionally ignores the reviewed `admin.html` fallback map and fails while CMS-owned admin Blade files still contain hard-coded user-visible English copy that should be moved to structured admin locale keys. A normal fallback-backed audit reaching 100% coverage is necessary for compatibility, but by itself it is not enough to remove `LocalizeAdminHtml`.
