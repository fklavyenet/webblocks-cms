...
- When CMS pre-renders a shared dialog layer under `#wb-overlay-root`, that layer must stay unhidden so WebBlocks UI `v2.7.12` can portal trusted modal and gallery targets into it without leaving them inside a hidden ancestor.
