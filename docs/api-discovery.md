...
Use `GET /webadmin/api/media` to discover existing CMS Media records before assigning media-backed native block fields. This read-only endpoint is moving to the dedicated `media.read` capability, with transitional `content.read` compatibility for older page-building tokens. For supported existing structured blocks, such as `navbar-brand` and `sidebar-brand`, use `PATCH /webadmin/api/blocks/{block}` to assign `media_id` and update safe settings or translations. Shared Slot source blocks additionally require `shared-slots.write`. Do not use Trusted HTML or invented public file paths when a native block contract exposes the needed media field.

The planned Media Library write surface is metadata-only at first: `PATCH /webadmin/api/media/{media}` with `media.write` may update `title`, `alt_text`, `caption`, and `description`. It must not upload, delete, replace files, move folders, change storage paths, or fetch remote media.
