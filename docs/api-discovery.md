...
Use `GET /webadmin/api/sites/{site}/assets/{css|js}` with `site-assets.read` and `PUT /webadmin/api/sites/{site}/assets/{css|js}` with `site-assets.write` to read or update the canonical physical `public/site/{site_handle}/css/site.css` and `public/site/{site_handle}/js/site.js` files. Read responses include `readiness` metadata so tools can see whether CMS can create or write the file and `guidance` metadata so tools can keep `site.css` token-first and mode-aware. Write requests send `contents` and `expected_checksum`; stale checksums are rejected so AI/operator tools must read, merge intentionally, and retry instead of overwriting concurrent edits. Hosting permission failures return JSON `422` with `errors.0.path = asset.write`, not HTML server errors.

For `site.css`, tools should prefer native block settings, CMS public theme custom properties, inherited `wb-*` component styling, and semantic site custom properties. Avoid page-wide hard-coded light backgrounds, dark text, white cards, or isolated dark-mode palettes when WebBlocks UI/CMS public theme tokens can express the design. Otherwise Light/Dark/Auto mode can render as a mixed page where some blocks follow WebBlocks UI and others are frozen by custom CSS.
