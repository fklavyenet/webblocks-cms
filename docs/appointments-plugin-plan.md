...
**0.4 Plugin blocks cannot own translatable fields.** *(Shipped in 1.58.0; adopted in plugin `0.10.0`.)* `BlockTranslationRegistry` was a fixed `match` over core slugs, so a plugin block had no translation family and no translated field map. The MVP works around this: visitor-facing copy comes from the plugin's own translation namespace, which is already supported — `InstalledPluginDefinitionFactory` registers `resources/lang` under the plugin handle — with per-block overrides held in block settings. Real per-block translation for plugin blocks arrived in CMS 1.58.0: a plugin block type declares `translatedFields([...])` and core stores each field per locale in rows. Plugin `0.10.0` declares `title`, `intro` and `submit_label`, which are the three pieces of copy an operator writes per placement.

The MVP workaround stays valid and stays in place underneath: plugin-shipped strings still belong in the plugin's own translation namespace, and a field with no translation row falls through to them — which is what makes a block nobody has opened render in the visitor's language rather than blank. What changed is that an operator can now give one placed block a second language, which the workaround never could.

Nothing else on the block is declared. A service id or a notification recipient means the same thing in every language, and per-locale rows for those would invite an operator to set two values that must not differ.
...
Phase 0.1 and 0.2 landed together in `1.43.0`, because they are the two halves of "a plugin can own a public surface" and neither is independently useful. Phase 0.3 followed in `1.43.1`. Plugin phases 1 through 6 shipped as `0.1.0` through `0.6.0`, completing the planned scope. `0.9.0` then added the progressive enhancement phase 3 had deferred, and `0.10.0` the per-placement translated copy phase 0.4 had been working around. Each moved the CMS floor with what it actually uses — `^1.57.0` for asset publishing, then `^1.58.0` for plugin block translations — so installs below those stay on `0.8.0` and `0.9.0` respectively, both complete.

Both remaining core phases have now shipped and both have been taken up: 0.6 in CMS `1.57.0`, adopted in plugin `0.9.0`; 0.4 in CMS `1.58.0`, adopted in `0.10.0`. Phase 0 is closed end to end — every gap this plan opened is not only fixed in core but in use by the plugin that found it.
