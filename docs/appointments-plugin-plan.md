...
Of the two core phases left open, 0.6 shipped in CMS 1.57.0 — a plugin can now serve its own CSS and JavaScript, so a progressive enhancement of the booking form is available whenever it is wanted. 0.4, per-block translated copy for plugin blocks, is still open and still blocks nothing that has shipped.
