...
Both remaining core phases have now shipped: 0.6 in CMS 1.57.0, so a plugin can serve its own CSS and JavaScript and a progressive enhancement of the booking form is available whenever it is wanted; and 0.4 in CMS 1.58.0, so the booking block's copy can be translated per placement rather than only per plugin release. Neither changes anything already shipped — both are now available rather than required.
