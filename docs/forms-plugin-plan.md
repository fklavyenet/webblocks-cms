...
Unlike appointments, this plugin had no blocking core gap at design time. Every contract it needed then had shipped in `1.43.x` and was exercised by three catalog plugins: declared block views (`admin_view` / `public_view`), `routes.public` under the reserved `/plugins/{handle}` prefix with `web`, `install.required`, and a mandatory group throttle applied by the registrar, plugin-owned admin routes with `plugin.permission:` middleware, permissions, migrations, settings with a plugin-named route, commands under the `{handle}:` prefix, and `resources/lang` registered under the plugin handle.

The floor has since moved past that, and the reason is worth separating from the sentence above: `0.6.0` ships a script, so it needs `^1.57.0` — the release that publishes a plugin's static files. That is a floor raised by something the plugin chose to add, not by a gap that blocked it. Everything through `0.5.0` still runs on `^1.45.6` and is complete.

Two gaps recorded as open in the appointments plan were still open when this was checked against `1.52.18`, and both shaped this design rather than blocking it. Both have since closed — 0.6 in CMS `1.57.0`, 0.4 in `1.58.0` — which completes phase 0 of the appointments plan and leaves no core extension point this design was written around still missing.

The entries below are kept in their original form with the resolution appended, because what they say about designing around a gap is the part worth keeping.

**0.4 Plugin blocks cannot own translatable fields.** *(Closed in CMS 1.58.0. Not adopted here, and correctly so.)* `BlockTranslationRegistry::familyFor()` was a fixed `match` over core slugs — `contact_form` is in it, a plugin handle cannot be. This plugin was less affected than appointments, because the workaround is the right design anyway: visitor-facing copy belongs to the *form*, not to the block that places it, so it lives in plugin-owned per-locale rows and never needs a block translation family. That reasoning is unchanged by 1.58.0 — a plugin block type can now declare `translatedFields([...])`, but this plugin's block still has exactly one setting and nothing to translate on it. The capability exists for plugins whose block *is* the copy, and the appointments booking block adopted it in its `0.10.0`.

Worth stating rather than leaving implicit: a closed gap is not an obligation. Declaring translated fields here would give an operator a per-locale editor for a select box, which is worse than nothing — it would imply the form's copy is edited on the block, when the whole design says it is not.
...
This was a real constraint on the product, and the honest response was to design around it rather than plan features that needed it. Conditional field visibility and multi-step forms are therefore **server-rendered**: a step is a GET/POST round trip and a conditional branch is evaluated on the server between steps, exactly as the appointments booking flow walks its steps. This costs a page load per step and gains something worth having — the form works with JavaScript off, each step is a shareable URL, and there is no client-side validation to drift out of sync with the server's.

That design outlived the constraint that produced it, and none of it was undone when 0.6 closed. The server still decides which fields were asked and which answers count; the script added in `0.6.0` only spares a visitor from seeing questions that do not apply to them.
...
`0.7.0` shipped the Contact Form importer, and how that came about is worth recording rather than tidying away. This document had described the importer in the present tense — "this plugin ships an importer" — since before v0.1, and at `0.6.0` it also claimed nothing remained unbuilt. Both sentences were in the same file and only one of them was true. The importer had never been written.

A plan that describes shipped software is read as a record, so a feature described in the present tense is one nobody goes looking for. The lesson is narrow and worth keeping: *shipped* belongs against a version number, and this section now carries one for every item.

The import reads core's blocks and writes only into this plugin's tables. That is the one direction across the plugin boundary that does not need permission it cannot have — reading a block to describe it changes nothing, and the coupling it does create (one block type, one settings bag, one translation table) fails at import time in front of the operator who asked for it, rather than at submit time in front of a visitor. Three shapes had to be translated: core keys block copy by locale id and this plugin by locale code; core's `content` is this plugin's `intro`; and `consent_label` is not form copy at all but the wording of the consent field, which is what carries it into a submission's schema snapshot. `0.7.0` did not move the CMS floor — every core API the import reads predates `1.57.0`.

One import decision is a deliberate departure from core rather than a copy of it. Core shows its consent checkbox only where the block has wording for the locale being rendered, so a block worded in one language silently asks visitors in the others nothing at all. A field here is not per-locale, so the import fills the gap from the default locale and **reports that it did**. It changes what a visitor is asked, which is exactly why it is not done quietly; the alternative, a required checkbox with no wording in some languages, asks a visitor to agree to nothing.
...
Replacing or migrating away from the native `contact_form` block is also out of scope, and this is now a settled decision rather than a recommendation. Core keeps Contact Form and Contact Messages exactly as they are. Plugin `0.7.0` ships an importer that reads an existing Contact Form block and creates an equivalent form definition from it, leaving the operator to swap the block on the page when they choose — and leaving the messages already in the core inbox where they are.
