...
- The shared public dialog layer must not be rendered with `hidden`; the pinned WebBlocks UI runtime reuses that layer for modal, gallery viewer, and toast targets and only toggles visibility on the active backdrop/modal or toast state, not on a reused layer wrapper
...
- `public.css` also owns the public shell's height model and the footer slot's default surface. `.wb-public-body` is a flex column with `min-height: 100vh` followed by `min-block-size: 100dvh`, and `.wb-public-body > .wb-slot-main` takes the leftover height, keeping the footer at the bottom of short pages. `dvh` tracks the visible mobile viewport rather than the largest one; the `100vh` line before it is the floor for browsers without `dvh`. Both are declared here because WebBlocks UI 2.18.0 removed the `min-height: 100vh` its reset used to put on `body` — on a `display: block` body it filled no height while still forcing the box to the large mobile viewport, which made a page with nothing to scroll scroll by the height of retracted browser chrome. The direct-child selector deliberately excludes the Docs shell, where `main` sits inside `.wb-dashboard-body` and that pattern owns its own height model. `.wb-slot-footer` takes `--wb-public-surface-strong` with a `--wb-public-border` top rule; both hooks are code-owned fixed classes rather than catalog `css_classes`, so a catalog re-sync cannot detach them, and site CSS still overrides them at equal specificity because it loads later
...
The Navigation tree's pinned SortableJS runtime is vendored under
`public/cms/js/vendor/` with its upstream license. The admin panel does not load
that executable dependency from a third-party CDN, so navigation editing does not
delegate administrator-origin script integrity to an external response.
