...
- The shared public dialog layer must not be rendered with `hidden`; WebBlocks UI `v2.7.6` reuses that layer for modal and gallery viewer targets and only toggles visibility on the active backdrop/modal state, not on a reused layer wrapper
...
CMS-owned default CDN references are pinned to WebBlocks UI `v2.7.6` for the public and admin runtime CSS, icons CSS, runtime JS, and the default icon manifest sync source.
