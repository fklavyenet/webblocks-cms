...
- The shared public dialog layer must not be rendered with `hidden`; WebBlocks UI `v2.7.17` reuses that layer for modal, gallery viewer, and toast targets and only toggles visibility on the active backdrop/modal or toast state, not on a reused layer wrapper
