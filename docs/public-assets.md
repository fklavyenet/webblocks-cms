...
- `public.css` also owns the public shell's height model and the footer slot's default surface. WebBlocks UI's reset gives `body` a `min-height: 100vh` that does nothing while `display` stays `block`, so `.wb-public-body` is a flex column with `min-block-size: 100dvh` and `.wb-public-body > .wb-slot-main` takes the leftover height, keeping the footer at the bottom of short pages. `dvh` tracks the visible mobile viewport rather than the largest one, and a browser without it falls back to the reset's `100vh` on its own. The direct-child selector deliberately excludes the Docs shell, where `main` sits inside `.wb-dashboard-body` and that pattern owns its own height model. `.wb-slot-footer` takes `--wb-public-surface-strong` with a `--wb-public-border` top rule; both hooks are code-owned fixed classes rather than catalog `css_classes`, so a catalog re-sync cannot detach them, and site CSS still overrides them at equal specificity because it loads later
