...
| Public theme presets and visual tones | Public site / Page Builder | Partially implemented / documented direction | Future `Sites -> Edit Site -> Theme`; initial `Icon tone` selects on supported icon-enabled block forms | Site-level theme tokens are planned; initial icon tone classes affect public block icons for supported blocks while blocks choose design roles such as `brand`, `accent`, or `quiet` | Content contract discovery exposes `settings.icon_tone` for supported blocks | Public theme is site-scoped and separate from admin UI preferences; semantic status tones remain separate from design tones | [docs/public-theme-and-tones.md](public-theme-and-tones.md), [docs/public-block-render-markup.md](public-block-render-markup.md) | site-level theme preset implementation docs after Phase 2 |
