...
| Public theme presets and visual tones | Public site / Page Builder | Documented direction / planned | Future `Sites -> Edit Site -> Theme` | Site-level theme tokens affect public block visual tones while blocks choose design roles such as `brand`, `accent`, or `quiet` | Planned discovery of tone fields through the content contract | Public theme is site-scoped and separate from admin UI preferences; semantic status tones remain separate from design tones | [docs/public-theme-and-tones.md](public-theme-and-tones.md) | implementation docs after Phase 1 |
