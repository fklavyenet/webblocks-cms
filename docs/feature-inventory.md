...
| Public theme presets and visual tones | Public site / Page Builder | Implemented | `Sites -> Edit Site -> Theme`; initial `Icon tone` selects on supported icon-enabled block forms | Public body emits `data-wb-public-theme="{preset}"` with `canvas` fallback; CMS public CSS maps all six presets to public page, surface, text, link/accent, button, badge, and icon tone tokens while blocks choose design roles such as `brand`, `accent`, or `quiet` | Content contract discovery exposes `settings.icon_tone` for supported blocks | Public theme is site-scoped and separate from admin UI preferences; semantic status tones remain separate from design tones; custom theme builder remains out of scope | [docs/public-theme-and-tones.md](public-theme-and-tones.md), [docs/public-block-render-markup.md](public-block-render-markup.md) | needs user guide |
| Site Assets | Public site | Implemented | `Sites -> Edit Site -> Assets` | Canonical physical `site.css` and `site.js` files render for the resolved public site when present | `GET/PUT /webadmin/api/sites/{site}/assets/{css\|js}` with `site-assets.read/write` | Admin/API-managed physical files at `public/site/{site_handle}/css/site.css` and `public/site/{site_handle}/js/site.js` with checksum conflict protection and revision snapshots before overwrite | [docs/public-assets.md](public-assets.md), [README.md](../README.md) | needs user guide |
...
| Engagement feedback API | AI operations | Implemented | `/webadmin/engagement/comments`, `/webadmin/engagement/ratings` | No direct public behavior beyond existing Rating/Comments blocks | `GET /webadmin/api/engagement/comments`, `GET /webadmin/api/engagement/ratings`, `PATCH /webadmin/api/engagement/comments/{comment}` | Requires explicit `engagement.read` for analysis and `engagement.moderate` for comment status changes; private visitor/IP/user-agent fields are not exposed | [docs/internal-content-api.md](internal-content-api.md), [docs/api-discovery.md](api-discovery.md) | covered |
