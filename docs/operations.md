...
The screen is status-first. `Update Status` is the only primary action area. When a compatible update is available, it shows the current-to-latest version path, compatibility state, the install CTA, and the backup download option. When no update is available, it shows a quiet up-to-date or unavailable message and omits package safety details.

Super admins have a persistent `System Updates` shortcut in the admin topbar. The shortcut checks update state asynchronously and shows its warning dot only when a newer trusted release is available.

Three compact safety cards show Publisher availability, preflight readiness, and backup protection. `Release` and `Readiness` are visible cards, not hidden accordion rows, so operators can scan package notes and install checks before applying an update. Low-level package safety remains in a collapsed `Package Safety Details` accordion only when an update is actionable. Super admins can download a support report from the history card for shared-hosting support workflows; the report avoids tokens, secrets, absolute local paths, and raw stack traces while including current/latest version, readiness, last-run details, and retained run summaries.
