...
`release:prepare` builds the package-rooted ZIP, checksum, and publisher payload locally. `release:publish-update` posts the package and metadata to the package-owned default publish endpoint `https://publisher.webblocksui.com/api/updates/publish`, then verifies latest update-server metadata for `webblocks-cms` on the selected channel. Installed CMS sites do not need Publisher/update server environment keys in normal `.env` files; CMS product code knows the default release server, product key, stable channel, latest path, and publish path. Publishing normally only needs `WEBBLOCKS_PUBLISHER_TOKEN`; `WEBBLOCKS_PUBLISHER_URL`, `WEBBLOCKS_PUBLISHER_PRODUCT`, and `WEBBLOCKS_PUBLISHER_CHANNEL` remain supported as explicit maintainer overrides. `WEBBLOCKS_UPDATES_SERVER_URL` and `WEBBLOCKS_UPDATES_CHANNEL` remain supported as operator overrides for installed update checks. Cached-config runs refresh canonical publisher keys from the project `.env`, diagnostics only report yes/no configured status, and legacy publisher aliases are intentionally unsupported. Git commits and tags are source-history steps only. GitHub Actions, GitHub releases, GitHub asset URLs, the GitHub API, and the `gh` CLI are not part of CMS update publishing, and `.github` workflows are intentionally absent.
