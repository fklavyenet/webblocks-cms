...
Update availability is based on the latest published release version being newer than the running CMS code version. Historical failed update runs and stale stored installed-version values remain inspectable through the run details modal, support report, retained-run CLI, or `Readiness`, but they do not make a current install actionable by themselves.

The screen is status-first. `Update Status` is the only primary action area. When a compatible update is available, it shows the current-to-latest version path, compatibility state, the `Update Now` CTA, and the backup download option. When no update is available, it shows a quiet up-to-date or unavailable message, avoids duplicate installed-version messaging, and omits package safety details.
...
Three compact safety cards show Publisher availability, preflight readiness, and backup protection only when an update is actionable, pending, incompatible, or needs attention. Package metadata, readiness checks, stored installed version, and retained update runs are grouped under `Technical details and history` for maintainers and support workflows instead of competing with the main CMS-user action. Low-level package safety remains in a collapsed `Package Safety Details` accordion only when an update is actionable. Super admins can download a support report from the history area for shared-hosting support workflows; the report avoids tokens, secrets, absolute local paths, and raw stack traces while including current/latest version, readiness, last-run details, and retained run summaries.

Update History renders retained runs on the main screen as a read-only table. Row deletion is not part of the admin UI. Update run records are pruned automatically, retaining the latest five runs by default. The latest failed run is preserved until a newer successful run exists. Operators with terminal access can inspect retained records with `php artisan webblocks:updates:runs`, `php artisan webblocks:updates:runs --last`, or `php artisan webblocks:updates:runs --failed`, and can run controlled pruning with `php artisan webblocks:updates:prune-runs --keep=5`.

The update client supports structured metadata from the update service, including title, summary, highlights, fixes, compatibility notes, migration notes, asset notes, operator notes, and technical notes. Native CMS release preparation derives summary and highlights from the matching `CHANGELOG.md` release section and adds release-specific technical metadata, so the Release card should describe the actual published version instead of repeated publishing workflow placeholders. These values are rendered as escaped plain text in operator-oriented groups inside the Release card. Older releases that only provide `release_notes` still show those notes cleanly, and releases without notes show `No release notes were provided for this release.` Readiness checks, stored installed version, and low-level update server values remain in Readiness.
