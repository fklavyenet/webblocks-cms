...
Detailed maintenance information is deliberately secondary. Publisher availability, preflight readiness, and backup protection are shown as compact safety cards only when an update is actionable, pending, incompatible, or needs attention. Package metadata, readiness rows, stored installed version, retained update runs, and support-report access live under `Technical details and history` for maintainers and support workflows. Low-level package safety stays in a compact accordion only when an update is actionable. Retained history keeps destructive row actions out of the UI and opens the latest run details in a modal when needed. The super-admin support report includes safe version/readiness/run summaries and excludes tokens, secrets, absolute local paths, and raw stack traces.
...
The `Release` details render structured metadata from fields such as `title`, `summary`, `highlights`, `fixes`, `compatibility_notes`, `migration_notes`, `asset_notes`, `operator_notes`, and `technical_notes`. The CMS renders those fields as escaped plain text and keeps readiness checks, stored installed version, and low-level response details in the technical details area.
