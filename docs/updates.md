...
Super admins also get a cached async update indicator in the admin top navbar when a newer trusted release is available. The indicator reads `/webadmin/system/updates/indicator` in the background and links to `Maintenance -> Update`, so normal admin page rendering is not blocked by the update server.
