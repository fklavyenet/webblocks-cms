...
Catalog list and detail pages are read-only. Artifact download and checksum values are guidance for manual operator review only; the CMS does not fetch, install, or apply catalog artifacts server-side. Manual ZIP upload/install remains the supported CMS plugin installation method, installed plugins remain disabled by default, setup/migrations stay explicit, and remote catalog metadata must not automatically install, enable, migrate, update, register routes, register permissions, register commands, register providers, register migrations, register assets, register blocks, add dashboard/system cards, change enabled state, or execute plugin code.
