...
`Install from Catalog` is available only when the catalog plugin is compatible, a latest compatible release exists, and the release includes `download_url`, `sha256`/`checksum_sha256`, and `filename`/`artifact_filename`. The POST action uses CSRF, re-reads catalog metadata server-side, downloads the controlled ZIP URL to temporary storage, checks HTTP success, rejects unreasonable/non-ZIP responses, calculates SHA-256, compares it exactly with catalog metadata, and then passes the temp ZIP through the existing manual plugin ZIP validator/installer. Temporary files are removed after success or failure, and catalog/remote filenames are not trusted as filesystem paths.
