...
`System -> Plugins` includes a `Browse Plugin Catalog` action at `/webadmin/plugins/catalog` (`admin.plugins.catalog.index`). The catalog list requests public listed plugins for `host_product=webblocks-cms` from `GET /api/plugins` and asks for the latest compatible release through `GET /api/plugins/{handle}/latest` when a handle is available. Catalog plugin labels and the View details action open `/webadmin/plugins/catalog/{handle}` (`admin.plugins.catalog.show`), which requests `GET /api/plugins/{handle}` plus the latest compatible release endpoint to show plugin metadata, compatibility, release notes, documentation/support links, artifact download URL, SHA-256 checksum, artifact filename/size, and safe declared capability metadata when returned by the API. The running CMS version is sent as `version` and `cms_version` for compatibility matching. The catalog base URL defaults to `https://plugins.webblocksui.com` and can be overridden with `WEBBLOCKS_PLUGIN_CATALOG_BASE_URL` (`webblocks-plugins.catalog.base_url`); timeout settings are available through `WEBBLOCKS_PLUGIN_CATALOG_TIMEOUT_SECONDS` and `WEBBLOCKS_PLUGIN_CATALOG_CONNECT_TIMEOUT_SECONDS`.

Catalog list and detail pages are read-only. Artifact download and checksum values are guidance for manual operator review only; the CMS does not fetch, install, or apply catalog artifacts server-side. The detail page helps operators review compatibility, open/download the ZIP externally, copy the download URL and SHA-256 checksum when safe values are provided, then continue through the existing manual ZIP upload screen. Any local installed/enabled state shown on the catalog detail page comes only from the CMS plugin registry, not from remote catalog claims. Manual ZIP upload/install remains the supported CMS plugin installation method, installed plugins remain disabled by default, setup/migrations stay explicit, and remote catalog metadata must not automatically install, enable, migrate, update, register routes, register permissions, register commands, register providers, register migrations, register assets, register blocks, add dashboard/system cards, change enabled state, or execute plugin code.
