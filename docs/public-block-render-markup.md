...
| settings.alternate_media_text_sections | true | Direct media/text child pairs are reordered by detected visual content while preserving the complete child collection and the `wb-grid` wrapper. Sibling alternating Grids under the same parent share one sequence, including across intervening non-alternating blocks. |
