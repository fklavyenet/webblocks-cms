...
Block contracts may expose optional public icon and badge fields, including `settings.icon_slug`, `settings.badge_tone`, and translated `eyebrow`/`badge_label` copy. Tools must treat icon slugs as catalog-backed selections rather than free text; public rendering skips unknown or inactive icons and only emits safe `wb-icon wb-icon-{slug}` classes for active WebBlocks UI catalog rows.
