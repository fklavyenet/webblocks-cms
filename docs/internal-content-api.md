...
Each personal token may also carry an optional allowlist of exact IPv4/IPv6 addresses or CIDR networks and its own 30, 60, 120, or 300 request-per-minute ceiling. An empty network list permits any client address, preserving existing-token behavior; tokens without a stored ceiling use 60 requests per minute. These live constraints are included in authenticated discovery under `token.network_policy`, and a token-specific limit may therefore be lower than the installation-wide API throttle.
...
Block contracts may expose optional public icon and badge fields, including `settings.icon_slug`, `settings.icon_tone`, `settings.icon_size`, `settings.badge_tone`, and translated `eyebrow`/`badge_label` copy. Tools must treat icon slugs as catalog-backed selections rather than free text; public rendering skips unknown or inactive icons and only emits safe `wb-icon wb-icon-{slug}` classes for active WebBlocks UI catalog rows. `settings.icon_tone` is a shared public visual tone setting, not locale-owned copy, and accepts only `default`, `soft`, `brand`, `accent`, `highlight`, `bold`, and `quiet`; `settings.icon_size` accepts `default`, `sm`, `lg`, or `xl` and maps to the matching WebBlocks UI size class. `info`, `success`, `warning`, and `danger` remain semantic status tones for badges, alerts, validation, and feedback. Unknown icon tones or sizes are rejected during content plan validation or ignored by public rendering as a safe fallback. Badge tone is exposed only when the block also owns translated badge-label copy.

`GET /webadmin/api/icon-catalog?context=content` returns the active safe icon slugs that API clients may assign to public content blocks. Use `context=navigation` to rank navigation-tagged suggestions first for navigation item icon choices; both contexts still return every active icon because context is a hint rather than an allowlist. API tools should read this endpoint before sending `settings.icon_slug` or navigation `icon` values and must not guess icon names from the WebBlocks UI manifest.
