...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `header` | Header | `content` | `resources/views/admin/blocks/types/header.blade.php` | `title` via text translation rows | `variant` heading level; `settings.alignment`; shared anchor in `settings.anchor` with legacy `url` fallback | Same-slot TOC reads anchored Header blocks | Not a container | `resources/views/pages/partials/blocks/header.blade.php` | Owns its root heading element | `clear` | `SyncCoreBlockTypesCommandTest`, `PublicEditorialBlocksRenderingTest`, `BlockTranslationIntegrityTest` | Canonical heading block; shared anchor contract is clear but still has legacy `url` fallback behavior. |
...
| `card` | Card | `layout` | `resources/views/admin/blocks/types/card.blade.php` | None | Shared `media_id`; `settings.layout_name`; `settings.background_position`; `settings.background_overlay`; optional `settings.url` and `settings.target` | Optional background image via direct `media_id`; child region blocks define structure | Container; allowed direct children are `card_header`, `card_body`, and `card_footer` only | `resources/views/pages/partials/blocks/card.blade.php` | Owns its `<article class="wb-card">` root or one semantic `<a class="wb-card wb-no-decoration">` root when linked | `clear` | `CardClickableRenderTest`, `PublicEditorialBlocksRenderingTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest`, `BlockTranslationIntegrityTest`, `MediaVisualBlockContractsTest` | Card is a composable shell. Its optional whole-card URL is safe and semantic; editors must not place nested interactive controls in linked Cards. It supports Media Library backgrounds and keeps a minimal legacy fallback for older rows without Card regions. |
| `card_header` | Card Header | `layout` | `resources/views/admin/blocks/types/card_header.blade.php` | None | `settings.layout_name`, `settings.icon_slug`, `settings.icon_tone`, `settings.icon_size` | Parent `card` relationship only | Container; may only be placed under `card`; card-region children are not allowed | `resources/views/pages/partials/blocks/card_header.blade.php` | Owns its `<div class="wb-card-header">` root | `clear` | `PublicEditorialBlocksRenderingTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest`, `PublicIconBadgeSupportTest` | Composable Card region block for header content. Optional icons use active content catalog slugs and render only through safe icon, tone, and size classes. Card Header has no badge copy and therefore exposes no badge tone. |
...
| `hero` | Hero | `content` | `resources/views/admin/blocks/types/hero.blade.php` | `title`, `subtitle`, `content` via text translation rows | Shared `media_id`; shared `variant`; `settings.layout`; `settings.title_tag`; `settings.background_position`; `settings.background_overlay` | Optional background image via direct `media_id`; child `button_link` blocks for actions | Container; `button_link` children (legacy `button` also allowed), no fixed count | `resources/views/pages/partials/blocks/hero.blade.php` | Owns its promo `<section>` root | `transitional` | `PublicHeroBlockRenderingTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest` | Hero is now a published source-backed core contract. Intro copy is locale-owned, action buttons are ordinary `button_link` children that own their own labels and URLs, and optional Media Library background images render through the native WebBlocks UI background-media primitive. Legacy content fallbacks remain readable where canonical translated fields are empty. |
...
| `column_item` | Column Item | `content` | `resources/views/admin/blocks/types/column_item.blade.php` | `title`, `eyebrow` as badge label, `subtitle`, `content` via text translation rows | Shared canonical `url`, `settings.icon_slug`, `settings.icon_tone`, `settings.icon_size`, `settings.badge_tone` | Parent `columns` relationship | Not a container | `resources/views/pages/partials/blocks/column_item.blade.php` | Parent-driven item root varies by Columns variant | `clear` | `PublicColumnsRenderingTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest`, `PublicIconBadgeSupportTest` | Column Item uses the shipped WebBlocks UI `wb-icon-card` composition for icon-led plain and card variants. Locale edits update item copy without overwriting shared URLs, icon slug, icon tone, icon size, or badge tone. |
| `cta` | CTA | `content` | `resources/views/admin/blocks/types/cta.blade.php` | `title`, `subtitle`, `content` via text translation rows | Shared `media_id`; shared `variant`; `settings.background_position`; `settings.background_overlay` | Optional background image via direct `media_id`; child `button_link` blocks for actions | Container; `button_link` children (legacy `button` also allowed), no fixed count | `resources/views/pages/partials/blocks/cta.blade.php` | Owns its promo `<section>` root | `clear` | `PublicHeroBlockRenderingTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest` | CTA is now a published promo contract with locale-owned copy, action buttons as ordinary `button_link` children, optional Media Library background images, and a root-owning public renderer. |
...
| `feature-item` | Feature Item | `content` | `resources/views/admin/blocks/types/feature-item.blade.php` | `title`, `eyebrow` as badge label, `content` via text translation rows | Shared canonical `url`, `settings.icon_slug`, `settings.icon_tone`, `settings.icon_size`, `settings.badge_tone` | Parent `feature-grid` relationship | Not a container | `resources/views/pages/partials/blocks/feature-item.blade.php` | Delegates to the Column Item cards presentation | `transitional` | `PublicHeroBlockRenderingTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest`, `PublicIconBadgeSupportTest` | Feature Item is now a published supporting child contract for Feature Grid while still sharing the existing Column Item cards and content-icon rendering. |
...
| `container` | Container | `layout` | `resources/views/admin/blocks/types/container.blade.php` | None | `settings.layout_name`; `settings.width`; `settings.flow` | Child blocks only | Container; no explicit child whitelist | `resources/views/pages/partials/blocks/container.blade.php` | Owns its root container `<div>` | `clear` | `PublicEditorialBlocksRenderingTest`, `PublicLayoutStructureTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest` | Container is layout-neutral when flow is unset or `none`; only explicit `Flow: Stack` adds vertical child flow. |
| `stack` | Stack | `layout` | `resources/views/admin/blocks/types/stack.blade.php` | None | `settings.layout_name`; `settings.spacing` | Child blocks only | Container; no explicit child whitelist | `resources/views/pages/partials/blocks/stack.blade.php` | Owns its root stack `<div>` | `clear` | `StackSplitCatalogTest`, `StackSplitContractTest` | Vertical child flow maps only to shipped `wb-stack` and `wb-stack-*` classes. |
| `split` | Split | `layout` | `resources/views/admin/blocks/types/split.blade.php` | None | `settings.layout_name`; `settings.gap`; `settings.items_alignment`; `settings.width` | Exactly two direct child blocks | Container; no explicit child-type whitelist, but direct child count is limited to two | `resources/views/pages/partials/blocks/split.blade.php` | Owns its root split `<div>` | `clear` | `StackSplitCatalogTest`, `StackSplitContractTest` | The first direct child grows and the second remains content-sized. A Stack may be nested on either side. The pinned UI has no responsive split-to-stack modifier yet. |
...
| `grid` | Grid | `layout` | `resources/views/admin/blocks/types/grid.blade.php` | None | `settings.layout_name`; `settings.columns`; `settings.gap`; `settings.alternate_media_text_sections`; `settings.alternate_start` | Child blocks only | Container; no explicit child whitelist | `resources/views/pages/partials/blocks/grid.blade.php` | Owns its root grid `<div>` | `clear` | `PublicEditorialBlocksRenderingTest`, `PublicLayoutStructureTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest` | Shared grid settings stay canonical. Alternating media/text grids keep the shipped `wb-grid-*` wrapper and reorder direct media/text child pairs, including Slider plus text Section structures, by detected visual content. Sibling alternating Grids under the same parent share one sequence even when other blocks appear between them, with the first alternating Grid controlling the start direction. |

### Pattern

| Slug | Label | Category | Admin form source | Translatable fields | Shared/settings fields | Media/relationship fields | Child/container behavior | Public renderer source | Renderer root contract | Current status | Tests / coverage | Known gaps / notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `content_header` | Content Header | `pattern` | `resources/views/admin/blocks/types/content_header.blade.php` | `title`, `eyebrow` as badge label, `subtitle`, `meta` via text translation rows | Shared `media_id`; `settings.alignment`, `settings.icon_slug`, `settings.icon_tone`, `settings.icon_size`, `settings.badge_tone`, `settings.background_position`, `settings.background_overlay` | Optional background image via direct `media_id`; `meta` is stored as structured list content | Not a container | `resources/views/pages/partials/blocks/content_header.blade.php` | Owns its root `<header>` | `clear` | `PublicEditorialBlocksRenderingTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest`, `BlockTranslationIntegrityTest`, `PublicIconBadgeSupportTest` | Content Header is a content-shell pattern block. It keeps locale-owned title, badge label, intro, and meta copy, always renders its title as H1, ignores any legacy saved heading-level values at render time, and keeps alignment, icon slug, icon tone, icon size, badge tone, and optional background media shared. |
...
| Slug | Label | Category | Admin form source | Translatable fields | Shared/settings fields | Media/relationship fields | Child/container behavior | Public renderer source | Renderer root contract | Current status | Tests / coverage | Known gaps / notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `link-list` | Link List | `navigation` | `resources/views/admin/blocks/types/link-list.blade.php` | `title`, `subtitle`, `content` via text translation rows | Optional `settings.row_layout` (index/stacked) and `settings.list_frame` (joined/cards) | Child `link-list-item` blocks | Container; only `link-list-item` children | `resources/views/pages/partials/blocks/link-list.blade.php` | Owns its `.wb-link-list` root when child rows exist, adding `wb-link-list--stacked` and `wb-link-list--cards` for the selected styles | `clear` | `LinkListBlockTest`, `PublicEditorialBlocksRenderingTest`, `BlockTranslationIntegrityTest`, `BlockTypePhaseThreeContractsTest` | Phase 3 now renders the existing translated intro copy above the public link list without changing child item behavior. |
| `link-list-item` | Link List Item | `navigation` | `resources/views/admin/blocks/types/link-list-item.blade.php` | Required `title`, optional `eyebrow` as badge label, optional `subtitle`, optional `content` via text translation rows | Required shared `url`, optional shared `media_id` thumbnail, optional `settings.icon_slug`, `settings.icon_tone`, `settings.icon_size`, `settings.badge_tone` | Optional thumbnail media through the direct block `media_id` relation; parent `link-list` relationship | Not a container | `resources/views/pages/partials/blocks/link-list-item.blade.php` | Owns its row link root, adds `wb-link-list-item--media` when a thumbnail or icon renders, and omits the description element when content is blank | `clear` | `LinkListBlockTest`, `LinkListThumbnailTest`, `PageBuilderExperienceTest`, `PublicEditorialBlocksRenderingTest`, `BlockTranslationIntegrityTest`, `PublicIconBadgeSupportTest` | Shared URL with translated row copy is consistent. Optional icons use active content catalog slugs and render only through safe icon, tone, and size classes. An optional image thumbnail renders as `wb-link-list-thumb` in the row's single leading column and wins over the icon when both are set. |
| `toc` | TOC | `navigation` | `resources/views/admin/blocks/types/toc.blade.php` | None | Canonical `title` only | Same-slot published `header` blocks with valid anchors | Not a container; historical child rows are preserved but new child placement is not supported | `resources/views/pages/partials/blocks/toc.blade.php` | Owns its generated TOC wrapper when headings exist | `clear` | `PublicRichContentTest`, `PageBuilderExperienceTest`, `TocHeadingScanTest` | TOC is a system block type. It scans only the slot it is placed in — not the whole page — walked in document order, and no longer treats arbitrary child blocks as public TOC content. |
...
| `header-actions` | Header Actions | `navigation` | `resources/views/admin/blocks/types/header-actions.blade.php` | None | Shared `settings.show_mode_toggle`; `settings.show_accent_toggle`; `settings.show_search`; `settings.show_language_switcher` | Search route plus client-side UI hooks | Not a container | `resources/views/pages/partials/blocks/header-actions.blade.php` | Owns its inner action cluster only | `clear` | `PublicEditorialBlocksRenderingTest` | System utility block; no translation ownership by design. |
...
- `hero`, `section`, `card`, `cta`, and `content_header` now share the Media Library background media contract: selected images stay on block `media_id`, position and overlay stay in shared settings, and public rendering emits CSS custom properties consumed by the native WebBlocks UI background-media primitive
- `rating` and `comments` are system engagement blocks: Rating stores idempotent 1-5 star rows in `content_ratings`, and Comments stores moderated visitor text in `comment_entries` with pending-by-default review and spam quarantine. Comments keeps its editorial heading in neighboring content blocks; Rating owns optional locale-aware `title` and `subtitle` copy through normal block text translations
- `rating` renders with the WebBlocks UI `wb-rating` primitive: the public block is `section.wb-card.wb-rating`, showing optional localized heading copy, a read-only `wb-rating-stars` average (partial fill via `--wb-rating-value`, plus `role="img"` and an average `aria-label`), and a no-JavaScript `wb-rating-input` star form where each star is a real submit button posting `rating_value` to `/content-ratings`. Behavior settings stay shared in block settings: `settings.scale` (fixed 5), `settings.allow_change`, and `settings.show_summary`; legacy `settings.title` remains a renderer fallback until the block is saved through the translated editor
- locale-only edits for structured child items now preserve shared URLs while still updating translated labels or copy
