...
| `grid` | Grid | `layout` | `resources/views/admin/blocks/types/grid.blade.php` | None | `settings.layout_name`; `settings.columns`; `settings.gap`; `settings.alternate_media_text_sections`; `settings.alternate_start` | Child blocks only | Container; no explicit child whitelist | `resources/views/pages/partials/blocks/grid.blade.php` | Owns its root grid or stack `<div>` | `clear` | `PublicEditorialBlocksRenderingTest`, `PublicLayoutStructureTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest` | Shared grid settings stay canonical. Normal grids render with shipped `wb-grid-*` classes; alternating media/text grids stack direct Section children and render each Section as a shipped `wb-grid wb-grid-2` row with media/text column order derived from Section order. |
