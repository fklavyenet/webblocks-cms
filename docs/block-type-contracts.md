...
| `card` | Card | `content` | `resources/views/admin/blocks/types/card.blade.php` | `eyebrow`, `title`, `subtitle`, `content`, `meta` via text translation rows; `image_alt`, `image_caption` via image translation rows | Shared `media_id`; `settings.url`; `settings.target`; `settings.variant`; `settings.image_position`; `settings.image_aspect` | Direct image media relation via `media_id`; child footer actions remain nested blocks and the legacy single-action fallback still uses translated `meta` | Container; allowed children are `cluster` and `button_link` | `resources/views/pages/partials/blocks/card.blade.php` | Owns its root card element | `transitional` | `PublicEditorialBlocksRenderingTest`, `PageBuilderExperienceTest`, `BlockTypePhaseThreeContractsTest`, `BlockTranslationIntegrityTest`, `MediaVisualBlockContractsTest` | Phase 3 keeps Card as a transitional content/container block: translated visible copy stays in text rows, optional image alt/caption use the existing image-translation path, selected media plus image presentation and action settings stay shared, no-image cards remain backward compatible, and nested child action composition plus the legacy single-action fallback both remain intentional. |
...
- `card` keeps its intentional transitional contract: translated visible copy stays in text rows, optional image alt/caption use the existing image-translation rows, shared `media_id` plus shared URL/target/variant/image presentation settings stay canonical, nested child action composition remains intact, and the legacy single-action fallback plus existing no-image rendering both remain documented and tested
