...
- `rating` and `comments` are system engagement blocks: Rating stores idempotent 1-5 star rows in `content_ratings`, and Comments stores moderated visitor text in `comment_entries` with pending-by-default review and spam quarantine. Comments keeps all visible copy in neighboring content blocks; Rating supports an optional single-language `settings.title` heading and otherwise composes visible copy with neighbors
- `rating` renders with the WebBlocks UI `wb-rating` primitive: the public block is `section.wb-card.wb-rating`, showing a read-only `wb-rating-stars` average (partial fill via `--wb-rating-value`, plus `role="img"` and an average `aria-label`) and a no-JavaScript `wb-rating-input` star form where each star is a real submit button posting `rating_value` to `/content-ratings`. Behavior settings stay in block settings: `settings.scale` (fixed 5), `settings.allow_change`, `settings.show_summary`, and the optional `settings.title`
