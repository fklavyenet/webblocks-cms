    ...
    private function documentedContracts(): array
    {
        return [
            ...
            'code' => [
                ...
                'translatable_fields' => [],
                'shared_settings_fields' => ['settings.language', 'canonical title', 'canonical subtitle', 'canonical content'],
                'storage_fields' => [
                    'Current code content is stored canonically on the block row.',
                ...
                'known_gaps' => [
                    'Admin copy presents translated fields, but `code` is not registered in `BlockTranslationRegistry`.',
            ...
            'stat-card' => [
                ...
                'storage_fields' => [
                    ...
                    'Optional URL is currently stored canonically on the block row.',
                ],
                'media_relationship_fields' => ['Not applicable.'],
                'child_container_behavior' => ['Not a container in the current contract.'],
                'renderer_root_contract' => 'Owns its public stat-card root.',
                'current_contract_status' => 'needs review',
                'known_gaps' => ['The admin form stores an optional URL, but the current public renderer does not use it.'],
            ],
            'table' => [
                'admin_form_fields' => ['Table Title', 'Table Style', 'Table Rows'],
                'translatable_fields' => [],
                'shared_settings_fields' => ['variant', 'canonical title', 'canonical content'],
                'storage_fields' => [
                    'Current table title and row payload are stored canonically on the block row.',
                ...
                'current_contract_status' => 'needs review',
                'known_gaps' => [
                    'Admin copy presents translated fields, but `table` is not registered in `BlockTranslationRegistry`.',
            ...
            'link-list' => [
                ...
                'current_contract_status' => 'transitional',
                'known_gaps' => ['Intro copy is editable and translated, but the current public renderer outputs only child rows.'],
            ],
            'link-list-item' => [
            ...
            'breadcrumb' => [
                ...
                'current_contract_status' => 'needs review',
                'known_gaps' => ['Renderer reads breadcrumb settings, but the shared BlockRequest path currently nulls settings for this slug, so admin form values do not persist.'],
            ],
            'header-actions' => [
