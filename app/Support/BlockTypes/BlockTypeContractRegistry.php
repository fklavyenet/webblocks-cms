...
class BlockTypeContractRegistry
{
    ...
    private function documentedContracts(): array
    {
        return [
            ...
            'card' => [
                'admin_form_fields' => ['Media', 'Image position', 'Image aspect', 'Eyebrow / Label', 'Title', 'Subtitle', 'Description', 'Action label', 'Image alt text', 'Image caption'],
                'translatable_fields' => ['eyebrow', 'title', 'subtitle', 'content', 'meta', 'image_alt', 'image_caption'],
                'shared_settings_fields' => ['media_id', 'settings.url', 'settings.target', 'settings.variant', 'settings.image_position', 'settings.image_aspect'],
                'storage_fields' => [
                    ...
                    'Shared URL, target, variant, image position, and image aspect stay in shared settings.',
                ...
                'known_gaps' => ['Current contract intentionally keeps both nested child actions and the older single-action fallback path while preserving no-image rendering for existing cards.'],
            ],
            'stat-card' => [
