...
return new class extends Migration
{
  public function up(): void
  {
    ...
    Schema::table('wbcms_system_update_runs', function (Blueprint $table) {
      if (! Schema::hasColumn('wbcms_system_update_runs', 'summary')) {
        $table->string('summary')->nullable()->after('status');
      }

      if (! Schema::hasColumn('wbcms_system_update_runs', 'warning_count')) {
        $table->unsignedInteger('warning_count')->default(0)->after('output');
      }

      if (! Schema::hasColumn('wbcms_system_update_runs', 'started_at')) {
        $table->timestamp('started_at')->nullable()->after('warning_count');
      }

      if (! Schema::hasColumn('wbcms_system_update_runs', 'finished_at')) {
        $table->timestamp('finished_at')->nullable()->after('started_at');
      }

      if (! Schema::hasColumn('wbcms_system_update_runs', 'duration_ms')) {
        $table->unsignedBigInteger('duration_ms')->nullable()->after('finished_at');
      }

      if (! Schema::hasColumn('wbcms_system_update_runs', 'triggered_by_user_id')) {
  ...
  public function down(): void
  {
    ...
    Schema::table('wbcms_system_update_runs', function (Blueprint $table) {
      ...
      $columns = collect(['summary', 'warning_count', 'started_at', 'finished_at', 'duration_ms'])
        ->filter(fn (string $column) => Schema::hasColumn('wbcms_system_update_runs', $column))
        ->all();
