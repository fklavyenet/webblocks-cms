...
class BlockTypeContractRegistry
{
  ...
  private function documentedContracts(): array
  {
    return [
      ...
      'split' => [
        'admin_form_fields' => ['Admin label', 'Gap', 'Align', 'Width'],
        'translatable_fields' => [],
        'shared_settings_fields' => ['settings.layout_name', 'settings.gap', 'settings.items_alignment', 'settings.width'],
      ...
      'hero' => [
        ...
        'storage_fields' => [
          ...
          'Optional hero image ownership stays on the canonical block media_id column. It renders as a background image for the left and centered layouts, and as a foreground image beside the copy when settings.layout is split.',
        ...
        'renderer_root_contract' => 'Owns its public promo `<section class="wb-card wb-promo">` root. The split layout adds `wb-promo--split` and renders the hero media as a `wb-promo-media` foreground image instead of a background.',
        'current_contract_status' => 'transitional',
      ...
      'rating' => [
        'admin_form_fields' => ['Heading', 'Vote changes', 'Public summary'],
        'translatable_fields' => [],
        'shared_settings_fields' => ['settings.scale', 'settings.allow_change', 'settings.show_summary', 'settings.title'],
        'storage_fields' => [
          'Optional single-language heading is stored in settings.title; leave empty to compose a heading with a neighboring Header block instead.',
        ...
        'child_container_behavior' => ['Not a container in the current contract.'],
        'renderer_root_contract' => 'Owns its public `section.wb-card.wb-rating` root, renders a read-only `wb-rating-stars` average and a no-JS `wb-rating-input` star form that posts each submit to `/content-ratings`, plus an optional `settings.title` heading.',
        'current_contract_status' => 'clear',
        'known_gaps' => [],
      ],
      'comments' => [
