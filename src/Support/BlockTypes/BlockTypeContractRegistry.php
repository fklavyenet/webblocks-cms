...
class BlockTypeContractRegistry
{
  ...
  private function documentedContracts(): array
  {
    return [
      ...
      'image' => [
        'admin_form_fields' => ['Media', 'Alt text', 'Link URL', 'Caption', 'Open in Gallery Viewer', 'Viewer Group'],
        'translatable_fields' => ['title', 'subtitle'],
        'shared_settings_fields' => ['media_id', 'url', 'settings.viewer_enabled', 'settings.viewer_group'],
        'storage_fields' => [
          ...
          'Viewer enablement and the page-local group key stay in shared block settings.',
        ],
        'media_relationship_fields' => ['Primary image media is owned through the direct block media_id relation.'],
        'child_container_behavior' => ['Not a container. Historical child rows may still exist, but new child placement is not supported and the public renderer ignores child trees.'],
        'renderer_root_contract' => 'Owns its semantic `<figure>` root when an image source exists. Unlinked images may join a named Gallery Viewer group through viewer_enabled and viewer_group.',
        'current_contract_status' => 'clear',
        'known_gaps' => [],
      ],
      'gallery' => [
      ...
      'section' => [
        ...
        'admin_form_fields' => ['Admin label', 'Spacing', 'Flow', 'Background media', 'Background position', 'Background overlay'],
        'translatable_fields' => [],
        'shared_settings_fields' => ['media_id', 'settings.layout_name', 'settings.spacing', 'settings.flow', 'settings.background_position', 'settings.background_overlay'],
        ...
        'renderer_root_contract' => 'Owns its public `<section>` root. Optional offset-up and overlap-previous flow modifiers create deliberate vertical rhythm and reset to normal flow on small screens.',
        'current_contract_status' => 'clear',
        'known_gaps' => [],
      ],
      'container' => [
      ...
      'stack' => [
        ...
        'storage_fields' => ['Shared vertical spacing stays in block settings.'],
        'media_relationship_fields' => ['Child blocks are the primary relationship.'],
        'child_container_behavior' => ['Container-capable. Child types are not explicitly restricted.'],
        'renderer_root_contract' => 'Owns its public stack `<div>` root.',
        'current_contract_status' => 'clear',
        'known_gaps' => [],
      ],
      'split' => [
        'admin_form_fields' => ['Admin label', 'Responsive behavior', 'Gap', 'Align', 'Width'],
        'translatable_fields' => [],
        'shared_settings_fields' => ['settings.layout_name', 'settings.gap', 'settings.items_alignment', 'settings.width', 'settings.responsive'],
        'storage_fields' => ['Shared two-sided layout settings stay in block settings.'],
        'media_relationship_fields' => ['Exactly two direct child blocks form the growing and content-sized sides.'],
        'child_container_behavior' => ['Container-capable. Accepts any renderable child type, with exactly two direct children required for the intended layout.'],
        'renderer_root_contract' => 'Owns its public split `<div>` root.',
        'current_contract_status' => 'clear',
        'known_gaps' => [],
      ],
      'cluster' => [
      ...
      'grid' => [
        'content_collection' => ['enabled' => true, 'child_types' => null],
        'admin_form_fields' => ['Admin label', 'Columns', 'Column ratio', 'Gap', 'Alternate media/text sections', 'First section layout'],
        'translatable_fields' => [],
        'shared_settings_fields' => ['settings.layout_name', 'settings.columns', 'settings.ratio', 'settings.gap', 'settings.alternate_media_text_sections', 'settings.alternate_start'],
      ...
      'hero' => [
        'admin_form_fields' => ['Eyebrow / Label', 'Title', 'Subtitle / Intro', 'Background media', 'Background position', 'Background overlay', 'Variant', 'Layout', 'Title tag'],
        'translatable_fields' => ['title', 'subtitle', 'content'],
        ...
        'storage_fields' => [
          ...
          'Optional hero image ownership stays on the canonical block media_id column. It renders as a background image for the left, centered, and full-bleed layouts, and as a foreground image beside the copy when settings.layout is split.',
          'Action buttons are ordinary child `button_link` blocks owned by their own admin form. Their labels are translated per locale and their URLs stay shared, exactly as for any other button_link.',
          'Internal Content API callers may still send primary_cta and secondary_cta objects ({label, url}) as a shorthand; the CMS appends the same `button_link` children an editor would add by hand. Declaring the buttons as explicit children is the canonical form.',
        ],
        'media_relationship_fields' => ['Optional hero background media is owned through the direct block media_id relation. Action buttons are stored as child block relationships.'],
        'child_container_behavior' => ['Container-capable. Allowed child types are `button_link` (and legacy `button`), with no fixed count.'],
        'renderer_root_contract' => 'Owns its public promo `<section>` root. Legacy layouts retain `wb-card wb-promo`; split adds `wb-promo--split` and foreground media. Full-bleed uses the unframed `wb-promo wb-public-hero--full-bleed` root and background media.',
        'current_contract_status' => 'transitional',
      ...
      'cta' => [
        'admin_form_fields' => ['Eyebrow / Label', 'Heading', 'Body Copy', 'Background media', 'Background position', 'Background overlay', 'Variant'],
        'translatable_fields' => ['title', 'subtitle', 'content'],
        ...
        'media_relationship_fields' => ['Optional CTA background media is owned through the direct block media_id relation. Action buttons are stored as child block relationships.'],
        'child_container_behavior' => ['Container-capable. Allowed child types are `button_link` (and legacy `button`), with no fixed count.'],
      ...
      'link-list' => [
        ...
        'storage_fields' => [
          ...
          'Shared row layout, list frame, and thumbnail size styles stay in block settings.',
        ...
        'renderer_root_contract' => 'Owns its public `.wb-link-list` root when published child rows exist, adding `wb-link-list--stacked`, `wb-link-list--cards`, and `wb-link-list--thumb-wide` for the selected styles.',
        'current_contract_status' => 'clear',
        'known_gaps' => [],
      ],
      'link-list-item' => [
        ...
        'storage_fields' => [
          ...
          'Optional thumbnail image ownership stays on the canonical block media_id column.',
          'Shared active catalog icon slug, icon tone, and badge tone stay in block settings.',
        ],
        'media_relationship_fields' => ['Optional row thumbnail media is owned through the direct block media_id relation. Parent `link-list` block is the primary relationship context.'],
        'child_container_behavior' => ['Not a container in the current contract.'],
        'renderer_root_contract' => 'Owns its public row-link root, adds the leading-visual modifier when a thumbnail or icon renders, and omits the description element when content is blank.',
        'current_contract_status' => 'clear',
        'known_gaps' => [],
      ],
      'toc' => [
      ...
      'rating' => [
        'admin_form_fields' => ['Heading', 'Supporting text', 'Vote changes', 'Public summary'],
        'translatable_fields' => ['title', 'subtitle'],
        'shared_settings_fields' => ['settings.scale', 'settings.allow_change', 'settings.show_summary'],
        'storage_fields' => [
          'Optional localized heading and supporting text live in block_text_translations rows.',
          'Legacy settings.title values remain a public-render fallback until the block is saved through the translated editor.',
        ...
        'child_container_behavior' => ['Not a container in the current contract.'],
        'renderer_root_contract' => 'Owns its public `section.wb-card.wb-rating` root, renders optional localized title/subtitle copy, a read-only `wb-rating-stars` average, and a no-JS `wb-rating-input` star form that posts each submit to `/content-ratings`.',
        'current_contract_status' => 'clear',
        'known_gaps' => [],
      ],
      'comments' => [
      ...
      'navigation-auto' => [
        ...
        'translatable_fields' => [],
        'shared_settings_fields' => ['settings.menu_key'],
        'storage_fields' => [
          'The selected CMS navigation menu handle lives in block settings as menu_key; settings.location is a legacy compatibility input.',
          'Visible link labels, URLs, targets, order, and visibility stay on Navigation records, not on this block.',
        ],
        'media_relationship_fields' => ['Current site and locale context resolve the rendered menu tree at render time.'],
        'child_container_behavior' => ['Not a container. Menu items come from Navigation records, not from child blocks.'],
        'renderer_root_contract' => 'Owns its public `<nav class="wb-stack wb-gap-2" data-wb-menu-key="...">` root and renders the visible menu tree as nested `<ul>` lists of `wb-link` anchors.',
        'current_contract_status' => 'clear',
        'known_gaps' => [],
      ],
      'breadcrumb' => [
