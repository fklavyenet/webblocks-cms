...
class BlockRequest extends FormRequest
{
  ...
  public function rules(): array
  {
    ...
    return [
      'page_id' => ['required', 'integer', 'exists:wbcms_pages,id'],
      'parent_id' => [
        'nullable',
        'integer',
        'exists:wbcms_blocks,id',
        Rule::notIn([$block?->id]),
      ],
      'block_type_id' => ['required', 'integer', 'exists:wbcms_block_types,id'],
      'slot_type_id' => ['required', 'integer', 'exists:wbcms_slot_types,id'],
      ...
      'section_flow' => [$supportsSectionSpacing ? 'nullable' : 'prohibited', Rule::in(['normal', 'offset-up', 'overlap-previous'])],
      ...
      'split_responsive' => [$isSplit ? 'nullable' : 'prohibited', Rule::in(['stack', 'preserve'])],
      ...
      'grid_ratio' => [$supportsGridColumns ? 'nullable' : 'prohibited', Rule::in(['equal', 'lead-left', 'lead-right'])],
      ...
      'gallery_asset_ids.*' => ['integer', 'exists:wbcms_media,id'],
      'gallery_items' => [$isGallery ? 'nullable' : 'prohibited', 'array'],
      'gallery_items.*.media_id' => [$isGallery ? 'required' : 'prohibited', 'integer', 'exists:wbcms_media,id'],
      ...
      'column_items.*.id' => ['nullable', 'integer', 'exists:wbcms_blocks,id'],
      'column_items.*.block_type_id' => ['nullable', 'integer', 'exists:wbcms_block_types,id'],
      ...
      'feature_items.*.id' => ['nullable', 'integer', 'exists:wbcms_blocks,id'],
      'feature_items.*.block_type_id' => ['nullable', 'integer', 'exists:wbcms_block_types,id'],
  ...
  public function validatedData(): array
  {
    ...
    if (! empty($data['block_type_id'])) {
      ...
      if ($blockType?->slug === 'columns') {
        ...
        $data['variant'] = $isTranslatedColumnsEdit
          ? ($this->route('block')?->getRawOriginal('variant'))
          : (in_array(trim((string) ($data['variant'] ?? 'plain')), ['cards', 'plain', 'stats'], true) ? trim((string) ($data['variant'] ?? 'plain')) : 'plain');
      }
      ...
      if (in_array($blockType?->slug, ['section', 'container', 'stack', 'split', 'cluster', 'grid', 'slider', 'slide'], true)) {
        ...
        $sectionFlow = trim((string) ($data['section_flow'] ?? 'normal'));
        ...
        $splitResponsive = trim((string) ($data['split_responsive'] ?? 'stack'));
        ...
        $gridRatio = trim((string) ($data['grid_ratio'] ?? 'equal'));
        ...
        if ($blockType->slug === 'section') {
          ...
          if (in_array($sectionFlow, ['offset-up', 'overlap-previous'], true)) {
            $settings['flow'] = $sectionFlow;
          } else {
            unset($settings['flow']);
          }
        ...
        if ($blockType->slug === 'grid') {
          ...
          if ($gridColumns === '2' && in_array($gridRatio, ['lead-left', 'lead-right'], true)) {
            $settings['ratio'] = $gridRatio;
          } else {
    ...
    foreach (($data['content_binding_selection'] ?? []) as $target => $selection) {
      if (! array_key_exists($target, $sourceEditor->targets($bindingBlock))) {
        continue;
      }
