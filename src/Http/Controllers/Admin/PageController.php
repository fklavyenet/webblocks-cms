...
class PageController extends Controller
{
  ...
  public function create(Request $request): View
  {
    ...
    return view('webblocks-cms::admin.pages.create', [
      'page' => new Page,
      'sites' => $sites,
  ...
  public function update(PageRequest $request, Page $page): RedirectResponse
  {
    ...
    $redirect = redirect()
      ->route('admin.pages.edit', array_filter([
        'page' => $page,
        'tab' => match ($request->input('_page_settings_tab')) {
          'assets' => 'page-assets',
          'overview' => 'overview',
  ...
  private function slotBlockModalState(Page $page, PageSlot $slot, $blocks, $blockTypes, $pickerBlockTypes, ?int $pickerParentId = null): array
  {
    ...
    $block->setAttribute('translation_state', in_array($selectedBlockType->slug, ['navigation-auto', 'menu', 'page-list', 'application'], true) ? 'shared' : 'missing');
  ...
  private function pickerBlockTypes($blocks, $blockTypes, ?int $parentId = null)
  {
    ...
    if (! $parentId) {
      return $blockTypes
        ->reject(fn (BlockType $blockType) => in_array($blockType->slug, ['sidebar-nav-item', 'sidebar-nav-group', 'card_header', 'card_body', 'card_footer', 'slide'], true))
        ->values();
    }
  ...
  private function sharedSlotsSchemaAvailable(): bool
  {
    return Schema::hasTable('wbcms_shared_slots')
      && Schema::hasTable('wbcms_shared_slot_blocks')
