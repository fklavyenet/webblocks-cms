  ...
  public function aiGuide(): JsonResponse
  {
    return response()->json([
      ...
      'content' => implode("\n\n", [
        ...
        'Embedded Application executable CSS/JS and its single managed HTML entry belong in application-scoped physical assets, not global site.css/site.js. List them with `GET /webadmin/api/sites/{site}/applications/{application}/assets`, then read or write `/assets/{css|js|html}/{filename}` with checksum protection. HTML accepts only `index.html` and returns the stable host-selected `/webblocks-applications/{application}/index.html` public path. A new file uses `expected_checksum: null`; replacement uses the checksum returned by GET. Register returned CSS/JS `public_path` values in the Embedded Application definition; writing the managed HTML entry activates its stable iframe URL. Filenames are safe basenames and extensions must match the selected type. Deletion requires `applications.delete`, preserves a revision, and is refused while the registry still references that path.',
