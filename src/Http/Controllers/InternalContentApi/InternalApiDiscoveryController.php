...
class InternalApiDiscoveryController extends Controller
{
  ...
  public function aiGuide(): JsonResponse
  {
    return response()->json([
      ...
      'content' => implode("\n\n", [
        ...
        'Embedded Application executable CSS/JS and its single managed HTML entry belong in application-scoped physical assets, not global site.css/site.js. Complete applications should use the package flow: PATCH `/applications/{application}` to increment its version, then POST a multipart ZIP in `package` to `/sites/{site}/applications/{application}/package`. The ZIP must contain root `index.html`; a successful install activates the returned immutable versioned `entry_url`. Use relative URLs inside the package. Installed versions cannot be overwritten. The legacy per-file endpoints remain available for CSS, JS, and one managed HTML entry.',
  ...
  private function links(): array
  {
    return [
      ...
      'page_versions' => '/webadmin/api/pages/{page}/versions',
      'page_version_review' => '/webadmin/api/pages/{page}/versions/{revision}',
      'page_version_candidate_prepare' => '/webadmin/api/pages/{page}/versions/{revision}/candidate',
      'page_version_candidate_apply' => '/webadmin/api/pages/{page}/version-candidates/{candidate}/apply',
      'page_version_candidate_discard' => '/webadmin/api/pages/{page}/version-candidates/{candidate}',
      ...
      'maintenance_cleanup' => '/webadmin/api/system/cleanup',
      'maintenance_cleanup_run' => '/webadmin/api/system/cleanup/{category}/run',
      ...
      'staged_update_discard' => '/webadmin/api/pages/{page}/staged-update',
      ...
      'plugin_catalog_update' => '/webadmin/api/plugins/catalog/{plugin}/update',
  ...
  private function openApiPaths(): array
  {
    ...
    return [
      ...
      '/content-sources' => ['get' => ['summary' => 'Discover accessible plugin content sources and fields', 'x-required-capability' => 'content.read', 'x-query-parameters' => ['block_id adds compatibility for one block', 'locale selects resolver context'], 'responses' => ['200' => ['description' => 'Content source contracts and entity record choices', 'content' => $json]]]],
      '/content-sources/{source}/preview' => ['post' => ['summary' => 'Preview one content source without saving a binding', 'x-required-capability' => 'content.read', 'x-supported-fields' => ['block_id', 'locale', 'record', 'limit', 'filter_field', 'filter_value', 'sort_field', 'sort_direction', 'source_settings'], 'responses' => ['200' => ['description' => 'Declared source fields only', 'content' => $json], '404' => ['description' => 'Source unavailable', 'content' => $json], '422' => ['description' => 'Invalid preview request', 'content' => $json]]]],
      ...
      '/system/cleanup' => [
        'get' => ['summary' => 'Read maintenance cleanup policy and category previews', 'x-required-capability' => 'maintenance.read', 'responses' => ['200' => ['description' => 'Policy and overview JSON', 'content' => $json]]],
        'put' => ['summary' => 'Replace maintenance cleanup retention settings', 'x-required-capability' => 'maintenance.settings.write', 'x-required-fields' => ['asset_revision_days', 'keep_latest_asset_revisions', 'temporary_workspace_hours', 'page_revision_days', 'keep_latest_page_revisions', 'shared_slot_revision_days', 'keep_latest_shared_slot_revisions'], 'responses' => ['200' => ['description' => 'Updated policy and overview JSON', 'content' => $json], '422' => ['description' => 'Validation JSON', 'content' => $json]]],
      ],
      '/system/cleanup/{category}/run' => ['post' => ['summary' => 'Run one safe maintenance cleanup category', 'x-required-capability' => 'maintenance.delete', 'x-destructive' => true, 'x-supported-categories' => ['asset-revisions', 'media-variants', 'temporary-workspaces', 'page-revisions', 'shared-slot-revisions'], 'responses' => ['200' => ['description' => 'Cleanup result JSON', 'content' => $json], '404' => ['description' => 'Unsupported category', 'content' => $json]]]],
      ...
      '/plugins/catalog/{plugin}/update' => ['post' => ['summary' => 'Update an installed plugin from a newer checksum-verified Plugin Catalog release', 'x-required-capability' => 'plugins.install', 'parameters' => [$pathParameter('plugin', 'Catalog plugin handle')], 'responses' => ['200' => ['description' => 'Updated plugin version JSON', 'content' => $json], '403' => ['description' => 'Requires plugins.install capability', 'content' => $json], '404' => ['description' => 'Installed or catalog plugin not found', 'content' => $json], '422' => ['description' => 'No newer compatible release or catalog update failed', 'content' => $json]]]],
      ...
      '/sites/{site}/applications/{application}/assets' => ['get' => ['summary' => 'List physical CSS, JavaScript, and managed HTML entry assets owned by one application on one site', 'x-required-capability' => 'applications.read', 'x-public-root' => '/site/{site_handle}/applications/{application_handle}/', 'responses' => ['200' => ['description' => 'Application asset list without file contents', 'content' => $json], '403' => ['description' => 'Requires applications.read capability', 'content' => $json]]]],
      '/sites/{site}/applications/{application}/package' => ['post' => [
        'summary' => 'Install and activate an immutable versioned Embedded Application ZIP package',
        'x-required-capability' => 'applications.write',
        'requestBody' => ['required' => true, 'content' => ['multipart/form-data' => ['schema' => ['type' => 'object', 'required' => ['package'], 'properties' => ['package' => ['type' => 'string', 'format' => 'binary']]]]]],
        'x-max-package-size' => '50 MB',
        'x-workflow' => ['Increment the application version with PATCH /applications/{application}', 'Create a ZIP with index.html at its root and relative asset URLs', 'Upload the ZIP as multipart field package', 'Use the activated entry_url returned by this response'],
        'responses' => ['200' => ['description' => 'Installed package and activated application definition', 'content' => $json], '403' => ['description' => 'Requires applications.write capability', 'content' => $json], '404' => ['description' => 'Application not found', 'content' => $json], '422' => ['description' => 'Missing, invalid, unsafe, oversized, or already-installed package version', 'content' => $json]],
      ]],
      '/sites/{site}/applications/{application}/assets/{type}/{filename}' => [
        'get' => ['summary' => 'Read one physical application asset', 'x-required-capability' => 'applications.read', 'x-supported-types' => ['css', 'js', 'html'], 'responses' => ['200' => ['description' => 'Application asset including contents and checksum', 'content' => $json], '403' => ['description' => 'Requires applications.read capability', 'content' => $json]]],
        'put' => ['summary' => 'Create or replace one physical application asset', 'x-required-capability' => 'applications.write', 'x-supported-types' => ['css', 'js', 'html'], 'x-required-fields' => ['contents', 'expected_checksum'], 'x-public-path' => '/site/{site_handle}/applications/{application_handle}/{type}/{filename}', 'x-managed-html-public-path' => '/webblocks-applications/{application_handle}/index.html', 'x-note' => 'Filename is a safe basename whose extension must match the type; html accepts only index.html and activates the stable managed iframe entry URL. Read first and send the returned checksum; null creates a missing file. Replaced files receive a revision snapshot.', 'responses' => ['200' => ['description' => 'Written application asset', 'content' => $json], '403' => ['description' => 'Requires applications.write capability', 'content' => $json], '422' => ['description' => 'Validation or stale checksum JSON', 'content' => $json]]],
      ...
      '/pages/{page}/staged-update' => ['delete' => ['summary' => 'Discard an active draft staged update', 'x-required-capability' => 'content.apply', 'x-note' => 'Deletes only the technical staged page. The published source page and its versions are unchanged. Successful promotion deletes the staged page automatically.', 'responses' => ['200' => ['description' => 'Discarded staged update JSON', 'content' => $json], '403' => ['description' => 'Requires content.apply capability', 'content' => $json], '422' => ['description' => 'Page is not an active draft staged update', 'content' => $json]]]],
      ...
      '/pages/{page}/versions' => ['get' => ['summary' => 'List saved page versions with change summaries', 'x-required-capability' => 'content.read', 'x-query-parameters' => ['page selects the result page', 'per_page accepts 1–100 and defaults to 50'], 'responses' => ['200' => ['description' => 'Paginated version list JSON', 'content' => $json]]]],
      '/pages/{page}/versions/{revision}' => ['get' => ['summary' => 'Review one saved page version against the current page', 'x-required-capability' => 'content.read', 'x-note' => 'Returns comparison, replacement scope, reference health, and an active restore candidate when present.', 'responses' => ['200' => ['description' => 'Version review JSON', 'content' => $json], '404' => ['description' => 'Version does not belong to page', 'content' => $json]]]],
      '/pages/{page}/versions/{revision}/candidate' => ['post' => ['summary' => 'Prepare an isolated restore candidate for preview', 'x-required-capability' => 'content.apply', 'x-note' => 'Does not change the current page. Returns the authenticated noindex preview URL.', 'responses' => ['201' => ['description' => 'Candidate JSON', 'content' => $json], '409' => ['description' => 'Version has missing required references', 'content' => $json]]]],
      '/pages/{page}/version-candidates/{candidate}/apply' => ['post' => ['summary' => 'Apply a reviewed restore candidate to the current page', 'x-required-capability' => 'content.apply + content.publish', 'x-destructive' => true, 'x-safety' => 'Refused when the source page changed after candidate preparation. Captures a pre-restore safety version before replacement.', 'responses' => ['200' => ['description' => 'Applied result JSON', 'content' => $json], '409' => ['description' => 'Candidate is stale or inactive', 'content' => $json]]]],
      '/pages/{page}/version-candidates/{candidate}' => ['delete' => ['summary' => 'Discard an isolated restore candidate', 'x-required-capability' => 'content.apply', 'x-note' => 'Deletes only the technical preview page; the current page and saved version are unchanged.', 'responses' => ['200' => ['description' => 'Discarded result JSON', 'content' => $json]]]],
