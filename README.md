...
- contact forms that store submissions first, then attempt synchronous email notification to a block-level recipient, site default recipient, `CONTACT_RECIPIENT_EMAIL`, or safe `MAIL_FROM_ADDRESS` fallback, with admin-visible delivery status and failure details for saved messages
...
- plugin system foundations for first-party and future package-owned capabilities, including compatibility metadata, collision guards, package conventions, and the disabled-by-default WebBlocks UI Manager pilot plugin for release metadata, artifact checksums, dry-run/apply local CDN publishing, plugin health, and read-only dashboard/system visibility when explicitly enabled
...
The plugin system treats CMS core as a plugin host for product-specific capabilities such as WebBlocks UI release/CDN management, QuizTem integrations, analytics, SEO pro tools, commerce, and custom block packs. The Phase 1 through Phase 5 runtime foundations now include the plugin registry, config-backed enabled state, compatibility metadata and required CMS version checks, enabled-only admin route and command registration conventions, read-only settings page scaffolding, health/status reporting with incompatible-plugin messaging, a super-admin `System -> Plugins` listing/detail surface, typed dashboard and system card extension slots, plugin-owned block declaration hooks, safe public asset contribution hooks for head and body-end assets, package convention/collision guards, and the first-party WebBlocks UI Manager pilot plugin. WebBlocks UI Manager is disabled by default and, when enabled and compatible, adds plugin-owned release/artifact/publish-run tables, `/webadmin/plugins/webblocks-ui-manager/...` release screens, safe local `webblocks-ui-manager:prepare-release` and `webblocks-ui-manager:publish-release` commands, checksum/manifest generation, dry-run/apply local CDN publishing into the configured static target, plugin health, and dashboard/system cards. It still does not include remote Composer discovery, generic plugin migrations/lifecycle UI, public plugin route discovery, a marketplace, generic third-party install/update flows, external production CDN deployment automation, update-server publishing, or CMS core WebBlocks UI URL changes. See `docs/plugin-system.md`.
...
- In-app System Updates now run the core catalog seed and block type sync automatically after migration handling and before the installed version is marked complete, so existing installs stay aligned with the current shipped CMS block, slot type, and page layout slot catalogs during upgrades. Package-native fresh Composer consumers do not run host Laravel application migrations during System Update; only dedicated package update migrations are considered.
...
- Admin chrome product identity is fixed to `WebBlocks CMS`, `A modern block-based CMS`, and `WebBlocks CMS v{VERSION}` from `WebBlocks\Cms\Support\WebBlocks`. System Settings and editable site fields do not change those labels.
