...
- `ddev artisan webblocks:package-status` reports the current package-transition resource boundary in a strictly read-only way and does not publish, migrate, clear cache, or mutate install state. `ddev artisan webblocks:package-status --view-check` additionally renders the internal package diagnostic Blade view through the `webblocks-cms::` namespace to verify package view loading without changing runtime ownership. The command now also reports package source authority, root compatibility wrappers, the root `public/cms` runtime asset compatibility path, root migration/update/install/auth blockers, and that package transition consolidation is complete for all safely movable CMS-owned source while starter split readiness remains blocked.
- Package transition consolidation is complete for all safely movable CMS-owned source in the in-repo package boundary. Active runtime asset URLs still use root `public/cms/...` compatibility paths, and the final remaining boundaries are install/auth, the app-owned `User` model, root migration authority, and future starter split work.
