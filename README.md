...
- Docs shell keeps the sidebar as the left shell region and groups header plus main content inside the right-side `.wb-dashboard-body.wb-w-full` column.
...
- Docs Navbar requires `wb-w-full` on both the navbar/header element and the inner flex alignment row.
- The navbar can use `wb-w-full` safely because it sits inside the content column, not as a direct flex sibling of `main` under `.wb-dashboard-shell`.
...
- Deleting a backup removes the backup record and deletes its stored archive file from the backups disk when that file is still present, including legacy records whose stored archive path still includes `backups/`, `storage/app/backups/`, or an absolute backups-disk path.
