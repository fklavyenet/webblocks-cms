...
- `docs/block-type-contracts.md` documents the Phase 1 inventory, Phase 2 read-only admin visibility, and the Phase 3 contract-alignment fixes for currently published core block type contracts, including the shipped media and visual block set `image`, `gallery`, `download`, `file`, `video`, and `audio`. `ddev artisan block-types:contracts-audit` gives a safe source-level audit of the same shipped contract source plus admin and public view presence.
