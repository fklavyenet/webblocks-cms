...
- `Page Layout` is now the editor-facing name for the page-level wrapper concept. The stored compatibility field remains `public_shell` for now, with existing handles such as `default` and `docs` preserved. A future managed Page Layout system may expose layout definitions and wrapper rules directly in the admin, but this release keeps the current stored handles and runtime mapping.
