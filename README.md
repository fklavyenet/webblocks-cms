...
- `ddev artisan webblocks:package-status` reports the current package-transition resource boundary in a strictly read-only way and does not publish, migrate, clear cache, or mutate install state. `ddev artisan webblocks:package-status --view-check` additionally renders the internal package diagnostic Blade view through the `webblocks-cms::` namespace to verify package view loading without changing runtime ownership. The command now also reports package-owned low-risk catalog seeders, root compatibility wrappers, moved low-risk runtime support helpers, the partial public model foundation moved into package `src/Models/`, active package admin and public runtime route authority, Site and Locale admin runtime package authority, operational admin runtime package authority including Slot Types and System Settings, shared admin partial/component package authority, root compatibility state for routes, views, migrations, and assets, package Composer metadata and provider discovery readiness, and the documented future Composer install or update target for the in-repo package boundary.
