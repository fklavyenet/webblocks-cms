...
6. For a docs-style context bar, add `Breadcrumb` and `Header Actions` to the `Header` slot. `Header Actions` renders system-owned theme utilities such as color mode, theme preset controls, accent controls, and the public search trigger without requiring raw HTML blocks.
...
`Edit Page -> Page Settings` now has `General` and `Page Assets` tabs. `General` keeps the current page fields. `Page Assets` is an advanced page-scoped feature for local `/site/...` CSS and JS files only. The tab uses a compact asset table with `Add CSS asset` and `Add JS asset` modal actions, and type is fixed by the selected add action rather than edited manually. In V1, Page Assets are stored relationally in `page_assets`, not in `pages.settings`, only `super_admin` users can change them, CSS renders in the public `<head>`, JS renders in the public `<head>` with `defer`, and the configured files are loaded only on the owning public page. Public block renderers must not emit inline scripts. Site handles are canonical lowercase hyphenated identifiers such as `ui-webblocksui-com`, and site/page override assets follow `public/site/{site_handle}/css/site.css`, `public/site/{site_handle}/js/site.js`, and `public/site/{site_handle}/pages/{page_slug}/page.{css,js}`. CMS-owned public JS belongs under `public/assets/webblocks-cms/js/`. Legacy public named JS rows saved with `body_end` remain backward compatible, but public named JS output is normalized to `<head defer>`. When site Export / Import runs with `Include media/assets`, referenced `/site/...` physical files are packaged and restored too. Without that option, the page asset rows still transfer but the target install must already have those public files.
...
- Admin form actions belong in the owning card or modal footer, not a separate action-only card. Use primary action first, cancel or secondary action second, and delete or destructive action last. Keep page headers focused on navigation and context actions instead of form submit actions.
