...
- a system-owned `Navbar` primitive block for reusable public header wrappers, plus composable `Navbar Brand` and `Navbar Navigation` child blocks
...
- `Navbar Brand` and `Navbar Navigation` are the intended child blocks for reusable header composition. `Navbar Navigation` reads relational `navigation_items` rows from the selected CMS Navigation menu; `Navbar Brand` renders logo and text using the shipped WebBlocks UI navbar classes.
