...
- in-app CMS updates now consume package-rooted release ZIPs and replace the transition `packages/webblocks-cms/` runtime plus the active Composer autoload package runtime when a consumer still loads WebBlocks CMS from `vendor/fklavyenet/webblocks-cms/...`, keeping the install-owned Laravel shell and root overrides intact
...
`Admin -> Maintenance -> Backups` supports selected bulk deletion for the backups visible on the current page. Use the leading row checkboxes or the header checkbox to select visible backups, then open `Delete selected` to review the count in a WebBlocks confirmation modal before submitting. The endpoint validates selected backup IDs again, deletes the records and archive files it can safely delete, skips unsafe items such as active running backups, and reports partial failures without exposing absolute storage paths. Backups single-delete actions use the same CMS modal pattern instead of browser confirmation dialogs.
...
- `ddev artisan webblocks:package-status` reports the current package-transition resource boundary in a strictly read-only way and does not publish, migrate, clear cache, or mutate install state. `ddev artisan webblocks:package-status --view-check` additionally renders the internal package diagnostic Blade view through the `webblocks-cms::` namespace to verify package view loading without changing runtime ownership. The command now also reports package source authority, root compatibility wrappers, the root `public/cms` runtime asset compatibility path, root migration/update/install/auth blockers, and that package transition consolidation is complete for all safely movable CMS-owned source, including the remaining moved support helpers that now live in package `src/Support/` while root `App\Support\...` entrypoints stay as compatibility wrappers.
- Package transition consolidation is complete for all safely movable CMS-owned source in the in-repo package boundary. That now includes the moved support helper implementations that previously still executed from root compatibility classes. Active runtime asset URLs still use root `public/cms/...` compatibility paths, and the final remaining boundaries are install/auth, the app-owned `User` model, root migration authority, and future starter split work.
- Package `public/cms/` now also carries admin CSS and JS source copies that match the current root runtime admin asset set, but the active admin layout still links to root `public/cms/...` URLs and brand assets remain root-owned for now.
...
- The public entry slice for page rendering, search, contact form submission, and privacy-consent sync now uses package-owned controllers, requests, and package namespace entry views, while root `App\Http\...` classes remain compatibility wrappers and broader public layouts, models, support code, and assets remain root-owned.
- The public model foundation now includes `Block`, `ContactMessage`, `Locale`, `Page`, `PageSlot`, `PageTranslation`, `PublicSearchIndex`, `Site`, `SiteDomain`, `SystemSetting`, and `VisitorEvent` in package `src/Models/`, with root `App\Models\...` wrappers preserved for compatibility. `User` remains app-owned and root-owned.
...
- The remaining safe operational admin route batch now also runs from the package for `Slot Types` and `System Settings`, with root controller, request, and Blade wrappers preserved so existing `App\...` imports and root view references continue to work.
- Package public assets and starter stubs now have real publishable package resources. Use `webblocks-cms-assets` for package assets and `webblocks-cms-stubs` for starter stubs; active runtime asset URLs still resolve through root `public/cms`, and root migrations plus current System Update behavior remain compatibility-authoritative until a dedicated install/update release flow replaces them.
...
- In-app System Updates now run the core block type catalog sync automatically after migration handling and before the installed version is marked complete, so existing installs stay aligned with the current shipped CMS block catalog during upgrades. Package-native fresh Composer consumers do not run host Laravel application migrations during System Update; only dedicated package update migrations are considered.
