...
- `docs/block-type-contracts.md` documents the Phase 1 inventory and Phase 2 read-only admin visibility for currently published core block type contracts, and `ddev artisan block-types:contracts-audit` gives a safe source-level audit of the same shipped contract source plus admin and public view presence.
