...
- `Page Layout` is now a managed install-level CMS concept under `Admin -> System -> Page Layouts`. Pages still store the selected layout handle on `public_shell` for backward compatibility, while the runtime resolves that handle safely through the managed Page Layout catalog. Built-in `Default Layout` and `Docs Layout` remain backward compatible with existing pages, imports, exports, Shared Slots, and public rendering. Custom V1 Page Layouts can map their handle to the existing `default` or `docs` shell behavior through `shell_type`, but Shared Slot compatibility remains conservative and exact by stored handle.
