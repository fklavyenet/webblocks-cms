...
- The public model foundation is now partial: `Locale`, `Site`, `SiteDomain`, `ContactMessage`, `PublicSearchIndex`, `VisitorEvent`, and `SystemSetting` are package-owned with root `App\Models\...` wrappers preserved for compatibility, while `Page`, `PageTranslation`, `PageSlot`, `Block`, and `User` remain root-owned for now.
