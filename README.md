...
On `Admin -> System -> Block Types`, the compact filters now separate metadata from live usage. `Support` filters capability and content-source metadata such as system-generated vs user-authored behavior plus admin, render, or container support, while `Usage` filters actual block usage counts so admins can compare used and unused block type rows on an install. The index pencil action now follows the shared query-driven admin modal pattern, so install-specific block types open `Edit Block Type: {Name}` in a modal and save back to the same filtered or paginated list context. The direct edit route remains available as a no-JavaScript fallback and uses the same named heading.
