...
- `Docs Navbar` aligns header slot children with shipped WebBlocks UI utilities such as `wb-cluster`, `wb-justify-between`, `wb-items-center`, and `wb-w-full` instead of spacer divs.
...
- `header-actions` uses semantic buttons plus a small CMS public JavaScript bridge to keep the mode icon synchronized and to drive an accent dropdown through shipped WebBlocks UI dropdown behavior and `WBTheme` state.
