...
- first-party WebBlocks UI Manager pilot plugin foundation for release metadata, artifact checksums, local manifest preparation, plugin health, and read-only dashboard/system visibility when explicitly enabled
...
The plugin system treats CMS core as a plugin host for product-specific capabilities such as WebBlocks UI release/CDN management, QuizTem integrations, analytics, SEO pro tools, commerce, and custom block packs. The Phase 1 through Phase 4 runtime foundations now include the plugin registry, config-backed enabled state, enabled-only admin route and command registration conventions, read-only settings page scaffolding, basic health/status reporting, a super-admin `System -> Plugins` listing/detail surface, typed dashboard and system card extension slots, plugin-owned block declaration hooks, safe public asset contribution hooks for head and body-end assets, and the first-party WebBlocks UI Manager pilot plugin. WebBlocks UI Manager is disabled by default and, when enabled, adds plugin-owned release/artifact tables, `/webadmin/plugins/webblocks-ui-manager/...` release screens, a safe local `webblocks-ui-manager:prepare-release` metadata command, checksum/manifest generation, plugin health, and dashboard/system cards. It still does not include dynamic Composer discovery, generic plugin migrations/lifecycle UI, public plugin route discovery, a marketplace, generic third-party install/update flows, or production CDN deployment. See `docs/plugin-system.md`.
