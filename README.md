...
- in-app CMS updates now consume package-rooted release ZIPs and replace the transition `packages/webblocks-cms/` runtime plus the active Composer autoload package runtime when a consumer still loads WebBlocks CMS from `vendor/fklavyenet/webblocks-cms/...`, keeping the install-owned Laravel shell and root overrides intact; pre-package-native installs such as `1.31.53` must first receive an old-shape root-managed bridge artifact that installs the package-native updater before they can consume package-rooted releases
