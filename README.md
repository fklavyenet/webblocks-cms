...
`Admin -> Media` now uses `Media` consistently as the admin-facing concept even though the internal model and storage layer still remain `Asset`-based in this phase. The `Media Library` list keeps preview as the eye-icon modal action, but the media title and pencil icon both open `Edit Media: {title}` with a safe return URL back to the current filtered list. That edit screen now merges preview, file details, metadata, organization, usage, and danger-zone management into one screen; `Copy public URL` lives in the `File Details` card, and delete uses the standard admin modal confirmation pattern instead of browser `confirm()`.
