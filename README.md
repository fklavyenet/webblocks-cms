...
- plugin system foundations for first-party and future package-owned capabilities, including compatibility metadata, collision guards, package conventions, and the disabled-by-default WebBlocks UI Manager pilot plugin for release metadata, artifact checksums, local manifest preparation, plugin health, and read-only dashboard/system visibility when explicitly enabled
