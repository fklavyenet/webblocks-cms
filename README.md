...
- Package transition consolidation is complete for all safely movable CMS-owned source in the in-repo package boundary. That now includes the moved support helper implementations that previously still executed from root compatibility classes, and a focused follow-up audit has removed several now-redundant root command and translation-model wrappers whose package implementations are already authoritative. Active runtime asset URLs still use root `public/cms/...` compatibility paths, and the final remaining boundaries are install/auth, the app-owned `User` model, root migration authority, and future starter split work.
