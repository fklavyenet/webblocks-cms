...
- Release WebBlocks Commerce `0.8.3` with required customer and delivery-address collection, order metadata snapshots and admin delivery review, plus an explicit no-payment test-order gateway that reserves inventory and leaves orders pending without contacting PayPal or SumUp. Route public buy actions through the cart so delivery details cannot be skipped.
- Release WebBlocks Commerce `0.8.2` with locale-aware currency symbols and separators, currency-specific fraction digits, Commerce and product currency selectors, PayPal/SumUp compatibility validation, and explicit PHP `ext-intl` documentation without a new Composer dependency. Align the legacy Commerce release harness version with its already-backported CMS 1.37.3 public bridge so the declared plugin compatibility floor is tested correctly.
