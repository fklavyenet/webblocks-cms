...
- **A tool can finally look at the page it built.** `GET /webadmin/api/pages/{page}/render` performs the same render as the browser admin preview: draft blocks included, noindex, never through the public route. `format=html` returns the markup, `locale` picks the translation. Content apply reports what was stored, which is not the same as what renders.
